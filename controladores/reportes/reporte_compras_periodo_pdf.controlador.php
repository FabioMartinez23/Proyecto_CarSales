...
if ($result && $result->num_rows > 0) {
    while ($r = $result->fetch_assoc()) {
        ...
        $cant  = (int)$r['cantidad_ingresos'];
        $total = (float)$r['total_tomado'];

        $total_ingresos_global += $cant;
...
if (count($filas) > 0) {
    foreach ($filas as $f) {
        ...
        $cantidad        = (int)$f['cantidad_ingresos'];
        $total_tomado    = (float)$f['total_tomado'];
        $valor_promedio  = (float)$f['valor_promedio'];

        $filas_html .= '
            <tr>
                <td style="padding:4px; border:1px solid #ccc;">' . $periodo . '</td>
                <td style="padding:4px; border:1px solid #ccc; text-align:center;">' . $cantidad . '</td>
                <td style="padding:4px; border:1px solid #ccc; text-align:right;">
                    $ ' . number_format($total_tomado, 2, ',', '.') . '
                </td>
                <td style="padding:4px; border:1px solid #ccc; text-align:right;">
                    $ ' . number_format($valor_promedio, 2, ',', '.') . '
                </td>
            </tr>
        ';
    }
...
$html .= '
    <div class="titulo-reporte">
        <h2 class="text-main">Reporte de Ingresos por Período (Mes / Año)</h2>
        <div class="subtitulo">
...
$html .= '
    <div class="kpi-container">
        <table class="kpi-table">
            <tr>
                <td class="kpi-label">Cantidad total de ingresos:</td>
                <td class="kpi-value">' . $total_ingresos_global . '</td>
            </tr>
            <tr>
                <td class="kpi-label">Total tomado:</td>
                <td class="kpi-value">$ ' . number_format($total_importe_global, 2, ',', '.') . '</td>
            </tr>
            <tr>
                <td class="kpi-label">Valor promedio por ingreso:</td>
                <td class="kpi-value">$ ' . number_format($valor_promedio_global, 2, ',', '.') . '</td>
            </tr>
        </table>
    </div>
...
// Podés cambiar a portrait si lo querés vertical
$dompdf->setPaper('A4', 'landscape');

$dompdf->render();
$dompdf->stream('reporte_ingresos_periodo.pdf', ['Attachment' => false]);
exit;
