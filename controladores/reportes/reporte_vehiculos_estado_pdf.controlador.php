...
// Datos POST
// ================================
$solo_activos = isset($_POST['solo_activos']) && $_POST['solo_activos'] == 1;
...
$result = $vehiculosModel->reporte_vehiculos_por_estado($solo_activos);
...
// KPIs por estado clave (opcional)
$map_estados = [];
    ...
    foreach ($estados as $e) {
        ...
        $filas_html .= '
            <tr>
                ...
                <td style="padding:4px; border:1px solid #ccc; text-align:center;">' . number_format($porc, 1, ',', '.') . '%</td>
                <td style="padding:4px; border:1px solid #ccc; text-align:right;">' . $dias . '</td>
            </tr>
        ';
    }
} else {
...
$html .= '
    <div class="titulo-reporte">
        <h2 class="text-main">Reporte de Vehículos por Estado</h2>
        <div class="subtitulo">
            Alcance: <strong>' . ($solo_activos ? 'Solo vehículos activos' : 'Todos los vehículos') . '</strong>
        </div>
    </div>
';
...
$html .= '
    <div class="kpi-container">
        <table class="kpi-table">
            <tr>
                <td class="kpi-label">Total de vehículos:</td>
                <td class="kpi-value">' . $total_vehiculos . '</td>
            </tr>
            <tr>
                <td class="kpi-label">Vehículos disponibles (aprox.):</td>
                <td class="kpi-value">' . $disp . '</td>
            </tr>
            <tr>
                <td class="kpi-label">Con falta de documentación/digitalización (aprox.):</td>
                <td class="kpi-value">' . $faltaD . '</td>
            </tr>
        </table>
    </div>
...
$dompdf->render();
$dompdf->stream('reporte_vehiculos_estado.pdf', ['Attachment' => false]);
exit;
