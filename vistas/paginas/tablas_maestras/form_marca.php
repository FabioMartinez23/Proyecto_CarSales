    ...
    <div class="col">
        ...
        <table class="table table-hover">
            ...
            <tbody>
                ...
                <td><?= $marca_['nombre']; ?></td>
                <td>
                    <a href="form_datos_para_vehiculos.php?page=form_marca&idmarcas=<?=$marca_['idmarcas']; ?>&nombre=<?=$marca_['nombre'];?>" class="btn btn-success" type="button">
                        <i class="fa-solid fa-pen-to-square"></i>
                    </a>
                </td>

                <td>
                    <form id="formulario-eliminar-<?= $marca_['idmarcas']; ?>" method="POST" action="../controladores/tablas_maestras/marca.controlador.php">
                        <input type="hidden" name="action" value="eliminar">
                        <input type="hidden" name="idmarcas" value="<?= $marca_['idmarcas'] ?>">
                        <button onclick="confirmarAccion(event, 'eliminar', 'formulario-eliminar-<?= $marca_['idmarcas']; ?>')" class="btn btn-danger" type="submit"><i class="fa-solid fa-trash"></i></button>
                    </form>
                </td>
                </tr>
                        <?php
        ...
        <nav aria-label="...">
            ...
            <li class="page-item <?php if($marcas->pagina_actual == 0){echo 'disabled';} ?>"> <!-- disabled -->
                <a class="page-link" href="form_datos_para_vehiculos.php?page=form_marca&pagina_actual=<?= $marcas->pagina_actual - 1 ?>">Previo</a>
            </li>
            <li class="page-item"><a class="page-link" href="form_datos_para_vehiculos.php?page=form_marca&pagina_actual=<?= $marcas->pagina_actual - 1 ?>"><?= $marcas->pagina_actual ?></a></li>
            <li class="page-item"><a class="page-link" href="form_datos_para_vehiculos.php?page=form_marca&pagina_actual=<?= $marcas->pagina_actual ?>"><?= $marcas->pagina_actual + 1 ?></a></li>
            <li class="page-item"><a class="page-link" href="form_datos_para_vehiculos.php?page=form_marca&pagina_actual=<?= $marcas->pagina_actual + 1 ?>"><?= $marcas->pagina_actual + 2 ?></a></li>
            <li class="page-item <?php if($marcas->pagina_actual == $total_registros - 1){echo 'disabled';} ?>">
                <a class="page-link" href="form_datos_para_vehiculos.php?page=form_marca&pagina_actual=<?= $marcas->pagina_actual + 1 ?>">Siguiente</a>
            </li>
            </ul>
        </nav>
    </div>
