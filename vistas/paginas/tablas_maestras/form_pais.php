    ...
    <div class="col">
        ...
        <table class="table table-hover">
            ...
            <tbody>
                ...
                <td>
                    <a href="form_datos_para_domicilios.php?page=form_pais&idpaises=<?=$pais_['idpaises']; ?>&nombre=<?=$pais_['descripcion'];?>" class="btn btn-success" type="button">
                        <i class="fa-solid fa-pen-to-square"></i>
                    </a>
                </td>

                <td>
                    <form id="formulario-eliminar-<?= $pais_['idpaises']; ?>" method="POST" action="../controladores/tablas_maestras/pais.controlador.php">
                        <input type="hidden" name="action" value="eliminar">
                        <input type="hidden" name="idpaises" value="<?= $pais_['idpaises'] ?>">
                        <button onclick="confirmarAccion(event, 'eliminar', 'formulario-eliminar-<?= $pais_['idpaises']; ?>')" class="btn btn-danger" type="submit"><i class="fa-solid fa-trash"></i></button>
                    </form>
                </td>
                </tr>
                        <?php
        ...
        <nav aria-label="...">
            ...
            <li class="page-item <?php if($paises->pagina_actual == 0){echo 'disabled';} ?>"> <!-- disabled -->
                <a class="page-link" href="form_datos_para_domicilios.php?page=form_pais&pagina_actual=<?= $paises->pagina_actual - 1 ?>">Previo</a>
            </li>
            <li class="page-item"><a class="page-link" href="form_datos_para_domicilios.php?page=form_pais&pagina_actual=<?= $paises->pagina_actual - 1 ?>"><?= $paises->pagina_actual ?></a></li>
            <li class="page-item"><a class="page-link" href="form_datos_para_domicilios.php?page=form_pais&pagina_actual=<?= $paises->pagina_actual ?>"><?= $paises->pagina_actual + 1 ?></a></li>
            <li class="page-item"><a class="page-link" href="form_datos_para_domicilios.php?page=form_pais&pagina_actual=<?= $paises->pagina_actual + 1 ?>"><?= $paises->pagina_actual + 2 ?></a></li>
            <li class="page-item <?php if($paises->pagina_actual == $total_registros - 1){echo 'disabled';} ?>">
                <a class="page-link" href="form_datos_para_domicilios.php?page=form_pais&pagina_actual=<?= $paises->pagina_actual + 1 ?>">Siguiente</a>
            </li>
            </ul>
        </nav>
    </div>
