    ...
    <div class="col">
        ...
        <table class="table table-hover">
            ...
            <tbody>
                ...
                <td>
                    <a href="form_datos_para_vehiculos.php?page=form_modelo_vehiculo&idmodelos=<?=$modelos_['idmodelos']; ?>&nombre=<?=$modelos_['nombre'];?>" class="btn btn-success" type="button">
                        <i class="fa-solid fa-pen-to-square"></i>
                    </a>
                </td>

                <td>
                    <form id="formulario-eliminar-<?= $modelos_['idmodelos']; ?>" method="POST" action="../controladores/tablas_maestras/modelo_vehiculo.controlador.php">
                        <input type="hidden" name="action" value="eliminar">
                        <input type="hidden" name="idmodelos" value="<?= $modelos_['idmodelos'] ?>">
                        <button onclick="confirmarAccion(event, 'eliminar', 'formulario-eliminar-<?= $modelos_['idmodelos']; ?>')" class="btn btn-danger" type="submit"><i class="fa-solid fa-trash"></i></button>
                    </form>
                </td>
                </tr>
                        <?php
        ...
        <nav aria-label="...">
            ...
            <li class="page-item <?php if($modelos_vehiculos->pagina_actual == 0){echo 'disabled';} ?>"> <!-- disabled -->
                <a class="page-link" href="form_datos_para_vehiculos.php?page=form_modelo_vehiculo&pagina_actual=<?= $modelos_vehiculos->pagina_actual - 1 ?>">Previo</a>
            </li>
            <li class="page-item"><a class="page-link" href="form_datos_para_vehiculos.php?page=form_modelo_vehiculo&pagina_actual=<?= $modelos_vehiculos->pagina_actual - 1 ?>"><?= $modelos_vehiculos->pagina_actual ?></a></li>
            <li class="page-item"><a class="page-link" href="form_datos_para_vehiculos.php?page=form_modelo_vehiculo&pagina_actual=<?= $modelos_vehiculos->pagina_actual ?>"><?= $modelos_vehiculos->pagina_actual + 1 ?></a></li>
            <li class="page-item"><a class="page-link" href="form_datos_para_vehiculos.php?page=form_modelo_vehiculo&pagina_actual=<?= $modelos_vehiculos->pagina_actual + 1 ?>"><?= $modelos_vehiculos->pagina_actual + 2 ?></a></li>
            <li class="page-item <?php if($modelos_vehiculos->pagina_actual == $total_registros - 1){echo 'disabled';} ?>">
                <a class="page-link" href="form_datos_para_vehiculos.php?page=form_modelo_vehiculo&pagina_actual=<?= $modelos_vehiculos->pagina_actual + 1 ?>">Siguiente</a>
            </li>
            </ul>
        </nav>
    </div>
