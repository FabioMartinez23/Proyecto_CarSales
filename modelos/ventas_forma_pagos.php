...
class VentaFormaPago {
    ...
    public function traer_vehiculo_forma_pago($ventas_idventas){
        $conexion = new Conexion();
        $query = "SELECT *, marcas.nombre as nombre_marca, modelos.nombre as nombre_modelo, tipo_vehiculos.nombre as nombre_tipo_vehiculo, colores.descripcion as nombre_color FROM ventas_forma_de_pago INNER JOIN compras ON ventas_forma_de_pago.compras_idcompras = compras.idcompras INNER JOIN vehiculos ON compras.vehiculo_idvehiculo = vehiculos.idvehiculos INNER JOIN modelos ON vehiculos.modelos_idmodelos = modelos.idmodelos INNER JOIN marcas ON modelos.marcas_idmarcas = marcas.idmarcas INNER JOIN tipo_vehiculos ON vehiculos.tipo_vehiculos_idtipo_vehiculos = tipo_vehiculos.idtipo_vehiculos INNER JOIN precios_vehiculos ON vehiculos.idvehiculos = precios_vehiculos.vehiculos_idvehiculos INNER JOIN colores ON vehiculos.colores_idcolores = colores.idcolores WHERE ventas_idventas = $ventas_idventas AND  activo_precio = 1";
