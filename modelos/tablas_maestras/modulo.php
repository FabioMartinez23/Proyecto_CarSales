...
class Modulos extends Paginacion{
    private $idmodulos;
    private $descripcion;
    ...
    public function traer_cantidad_modulo(){
        $conexion = new Conexion();
        $query = "SELECT count(*) as total FROM modulos WHERE activo_modulo = 1";
        return $conexion->consultar($query);
    }

    public function traer_modulos_paginacion(){
            $conexion = new Conexion();
            $query = "SELECT * FROM modulos WHERE activo_modulo = 1 LIMIT $this->pagina_actual,$this->paginacion";
            return $conexion->consultar($query);
    }
