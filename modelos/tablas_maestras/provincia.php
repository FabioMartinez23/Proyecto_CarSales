...
class Provincias extends Paginacion{
    ...
    public function traer_provincias_por_pais($paises_idpaises){
        $conexion = new Conexion();
        $query = "SELECT provincias.*, provincias.descripcion as nombre_provincia, paises.*, paises.descripcion as nombre_pais FROM provincias INNER JOIN paises on provincias.paises_idpaises = paises.idpaises WHERE paises.idpaises = $paises_idpaises";
        return $conexion->consultar($query);
    }

    public function traer_provincia_id($idprovincias){
        $conexion = new Conexion();
        $query = "SELECT * FROM provincias WHERE idprovincias = '$idprovincias'";
        return $conexion->consultar($query);
    }
